...
exports.run = async (client, message, args) => {
  ...
  message.author.send(`Qual título quer dar a este anúncio?`).then(async (msg_suges) => {
  ...
  message.author.send(`Qual descrição deseja dar ao anúncio?`).then(async (msg_suges1) => {
  ...
  message.author.send(`Qual cor você deseja no anúncio? (Envie no formato #HEX)`).then(async (msg_suges2) => {
  ...
  message.author.send(`Qual id do canal que você deseja enviar o anúncio?`).then(async (msg_suges) => {
  let collector = msg_suges.channel.createMessageCollector(m => m.author.id === message.author.id, { max: 1, time: 50000000 })
  ...
  message.author.send(`<:certo:678374718587535402> Voce conseguiu fazer um anúncio!`)
  const canal = client.channels.get(`${id}`).send("@everyone", embed)
